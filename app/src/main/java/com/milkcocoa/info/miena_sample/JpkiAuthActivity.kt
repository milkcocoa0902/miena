...
import com.milkcocoa.info.miena.jpki.authenticate.JpkiAuth
...
class JpkiAuthActivity: AppCompatActivity(){
    companion object{
        private const val TAG = "JPKI Auth"
    }

    val jpki by lazy { JpkiAuth() }
            ...
            readCertificace.setOnClickListener {
                lifecycleScope.launch {
                    ...
                    NfcAdapter.ReaderCallback { tag->
                        lifecycleScope.launch {
                            kotlin.runCatching {
                                ...
                                createDocument.launch("jpki.der")?.let { contentResolver.openOutputStream(it) }?.use { outputStream->
                                    ...
                                    Toast.makeText(this@JpkiAuthActivity, "証明書を保存しました", Toast.LENGTH_SHORT).show()
                                }
                            }.getOrElse {
                                when(it){
                                    is NoVerifyCountRemainsException ->{
                                        Toast.makeText(this@JpkiAuthActivity, "カードがロックされています", Toast.LENGTH_SHORT).show()
                                    }
                                    is AdpuValidateException ->{
                                        Toast.makeText(this@JpkiAuthActivity, "カードの読み取りに失敗しました", Toast.LENGTH_SHORT).show()
                                    }
                                }
                            }.also {
                                nfcCard.disableReaderMode(this@JpkiAuthActivity)
                        ...
                        nfcCard.enableReaderMode(this@JpkiAuthActivity, nfcCallback, NfcAdapter.FLAG_READER_NFC_B or NfcAdapter.FLAG_READER_SKIP_NDEF_CHECK, null)
                    }
                }
            }
        }
                        ...
                        lifecycleScope.launch {
                            runCatching {
                                ...
                                if(remains > 0){
                                    ...
                                    // 6. 秘密鍵で署名する
                                    val signature = this@JpkiAuthActivity.contentResolver.openInputStream(uri!!)?.use {inputStream->
                                    ...
                                    createDocument.launch("signature.sig")?.let { contentResolver.openOutputStream(it) }?.use { outputStream->
                                        ...
                                        Toast.makeText(this@JpkiAuthActivity, "署名を保存しました", Toast.LENGTH_SHORT).show()
                                    }
                                }else{
                                    Toast.makeText(this@JpkiAuthActivity, "カードがロックされています", Toast.LENGTH_SHORT).show()
                                }
        ...
        findViewById<Button>(R.id.verify).also { verifySignature->
            verifySignature.setOnClickListener {
                lifecycleScope.launch {
                    ...
                    Toast.makeText(this@JpkiAuthActivity, "証明書を選択してください", Toast.LENGTH_SHORT).show()
                    val cert = selectFile.launch() ?: return@launch

                    Toast.makeText(this@JpkiAuthActivity, "検証したいファイルを選択してください", Toast.LENGTH_SHORT).show()
                    val file = selectFile.launch() ?: return@launch

                    Toast.makeText(this@JpkiAuthActivity, "検証するファイルの署名を選択してください", Toast.LENGTH_SHORT).show()
                    ...
                    if(verify){
                        Toast.makeText(this@JpkiAuthActivity, "選択したファイルと署名は、証明書によりただしく検証されました", Toast.LENGTH_SHORT).show()
                    }else{
                        Toast.makeText(this@JpkiAuthActivity, "選択したファイルと署名が一致しませんでした", Toast.LENGTH_SHORT).show()
                    }
                }
            }
        }
            ...
            DialogFragmentPinCode(
                object : DialogFragmentPinCode.DialogFragmentPinCodeListener{
                    ...
                    override fun onComplete(pin: String) {
                        if(pin.isNotBlank() and pin.isDigitsOnly()){
